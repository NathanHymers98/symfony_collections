...
class GenusFormType extends AbstractType // This is the Genus form
{
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        $builder
            ...
            // Adding another field to the form which will allow us to change which user is related to a specific genus, depending on which on is selected.
                // It also shows which users are already directly related to that specific genus
            ->add('genusScientists', CollectionType::class, [
                'entry_type' => GenusScientistEmbeddedForm::class,
                'allow_delete' => true, // Allow delete set to true means that it is ok if one of the GenusScientist fields are missing from the submitted data. e.g. one of them is set to null (Which will delete it when using orphanRemoval)
                'allow_add' => true, // If there is suddenly an extra set of new GenusScientist form data that is submitted, that is fine
                'by_reference' => false,
            ])
        ;
